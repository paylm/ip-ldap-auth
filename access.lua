...
local ldap = require "kong.plugins.ip-ldap-auth.ldap"
local ip = require "kong.plugins.ip-ldap-auth.ip"
...
local function do_ip_check(conf)
  local binary_remote_addr = ngx.var.binary_remote_addr
  if not binary_remote_addr then
    return false, {status = 403, message = "Cannot identify the client IP address, unix domain sockets are not supported." }
  end

  if conf.deny and #conf.deny > 0 then
    local blocked = ip.match_bin(conf.deny, binary_remote_addr)
    if blocked then
      return false, {status = 403, message = "Your IP address is not allowed" }
    end
  end

  if conf.allow and #conf.allow > 0 then
    local allowed = ip.match_bin(conf.allow, binary_remote_addr)
    if not allowed then
      return false, {status = 403, message = "Your IP address is not allowed" }
    end
  end
